...
def load(filename=None,
        ...
        loader_class=None):
    """load google's `emoji4unicode` project's xml file. must call this method first to use `e4u` library. this method never work twice if you want to reload, use `e4u.reload()` insted."""
    if not has_loaded():
...
def reload(filename=None,
        url=r"http://emoji4unicode.googlecode.com/svn/trunk/data/emoji4unicode.xml",
        loader_class=None):
    """reload google's `emoji4unicode` project's xml file. must call this method first to use `e4u` library."""
...
def has_loaded():
    """get has `e4u.load()` method called or not."""
    return _loader != None
...
def translate_char(source_char, carrier, reverse=False, encoding=False):
    """translate unicode emoji character to unicode carrier emoji character (or reverse)
    ...
    """
    if not isinstance(source_char, str) and encoding:
        source_char = source_char.decode(encoding, 'replace')
    elif not isinstance(source_char, str):
        raise AttributeError("`source_char` must be decoded to `unicode` or set `encoding` attribute to decode `source_char`")
    if len(source_char) > 1:
        raise AttributeError("`source_char` must be a letter. use `translate` method insted.")
...
def translate(source, carrier, reverse=False, encoding=None):
    """translate unicode text contain emoji character to unicode carrier text (or reverse)
    ...
    """
    if not isinstance(source, str) and encoding:
        source = source.decode(encoding, 'replace')
    elif not isinstance(source, str):
        raise AttributeError("`source` must be decoded to `unicode` or set `encoding` attribute to decode `source`")
